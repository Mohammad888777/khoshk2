        ...
        if new_value > self.bucket_capacity:
            ...
            if request.is_ajax():
                if request.method=="GET":
                    messages.error(request,"دسترسی شما محدود شده است")
                    return redirect("home")   
                else:
                    return JsonResponse({"many":True ,"url":f"/"})
            else:
                if request.method=="GET":

                    messages.error(request,"دسترسی شما محدود شده است لطفا بعد امتحان کنید")
                    return redirect("home")
